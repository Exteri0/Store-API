...
async function addProduct(req:Request,res:Response){
    ...
    try{
        const result = await store.addProduct(req.body.product_id,req.body.quantity,req.body.order_id);
        res.status(200).json(result);
    }
    catch(err){
        res.status(500).json(`internal server error ${err}`)
...
async function setStatus(req:Request,res:Response){
    ...
    try{
        const result = await store.setStatus(req.body.status,req.params.id as unknown as number);
        res.json(result);
    }catch(err){
        res.status(500).json(`internal server error ${err}`)
    }
...
export async function OrderRoutes(app:express.Application){
    ...
    app.get("/orders/:id",function(req:express.Request,res:express.Response){
        currentOrder(req,res);
    })
    app.post("/orders",function(req:express.Request,res:express.Response){
        create(req,res);
    })
    app.post("/orders/add",function(req:express.Request,res:express.Response){
        addProduct(req,res);
    })
    app.post("/orders/:id",function (req:express.Request,res:express.Response){
        setStatus(req,res);
    })
}
